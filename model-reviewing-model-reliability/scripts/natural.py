...
def review_all():
    ...
    for path in sorted((ROOT / "transcripts").glob("*.txt")):
        ...
        path_out = RESULTS / "reviewed.json"
        path_out.write_text(json.dumps(drafts, indent=2))
        print(f"  reviewed {path.stem}: {len(claims)} claims")
    return drafts


def score_all(drafts):
    ...
    for stem, d in drafts.items():
        ...
        for claim in d["claims"]:
            ...
            for version in ("free_rewrite",):
                row[f"survives_{version}"] = supported(d[version], claim["text"])
            rows.append(row)
        print(f"  scored {stem}")
    (RESULTS / "claims.json").write_text(json.dumps(rows, indent=2))
    return rows
...
def main():
    ...
    drafts = review_all()
    for stem, d in drafts.items():
        (RESULTS / "outputs" / f"{stem}.txt").write_text(d["free_rewrite"])
    rows = score_all(drafts)
    print(f"\n{'draft':<7}{'id':<4}{'section':<14}{'checker':<8}{'free rewrite':<14}claim")
    for r in rows:
